...
3. **Таблица без владельца не заводится.** Проверяется машиной:
   ...
   создана мимо карты. Исключение одно — таблицы самого механизма миграций
   (`schema_version`): они не про предметную область, перечислены поимённо в
   `scripts/migration_model.py` и там же объяснены.
