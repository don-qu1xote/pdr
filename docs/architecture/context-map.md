...
* адаптер billing обращается к таблице `tariffs`
  ...
  а карта требует имени `billing_tariff`. Самой таблицы в схеме нет: миграции
  завели только контекст `identity` (`PDR-DB-02`). Имя придёт к владельцу
  вместе с миграцией, заводящей тарифы. Сама проверка тут не поможет и не
  притворяется, что поможет: `scripts/check_table_owners.py` сверяет
  `create table` в миграциях, а не имена таблиц в запросах, — поэтому
  расхождение записано здесь.
