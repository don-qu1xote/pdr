...
фазой, а не все девятнадцать в первый день.
...
flowchart TD
    ...
    class identity,scheduling,billing,notifications,jobs exists;
...
Сплошным помечены пять контекстов первой фазы: у них есть модуль в `libs/`
...
очереди, у jobs — механизм одиночных периодических заданий. Остальное владение
из списков ниже — план, а не код.

Стрелок у `jobs` нет, и это не упущение: он ничего ни у кого не читает.
Зависимость идёт в другую сторону и не является чтением данных — контекст,
которому нужно периодическое задание, реализует его контракт.
...
Девятнадцать: семнадцать доменных и два технических — `ml` и `jobs`. Они ничего
не решают про предметную область и доменными данными не владеют, но границу
имеют такую же, поэтому стоят на карте на общих правах.
