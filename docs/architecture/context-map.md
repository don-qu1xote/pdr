...
фазой, а не все двадцать в первый день.
...
flowchart TD
    ...
    jobs["jobs<br/>фаза 1 · технический · есть модуль"]
    observability["observability<br/>фаза 1 · технический · есть модуль"]
    ...
    class identity,scheduling,billing,notifications,jobs,observability exists;
...
Сплошным помечены шесть контекстов первой фазы: у них есть модуль в `libs/`
со своей целью сборки, своими слоями и своими тестами (`PDR-ARCH-07`,
`PDR-DB-03`). Остальные существуют только здесь, на карте, и модуля у них нет
намеренно: модуль под контекст, который родится через три фазы, — это мусор,
...
очереди, у jobs — механизм одиночных периодических заданий, у observability —
продуктовое событие и его обезличивание. Остальное владение из списков ниже —
план, а не код.

Стрелок у `jobs` и `observability` нет, и это не упущение: они ничего ни у кого
не читают. Зависимость идёт в другую сторону и не является чтением данных —
контекст, которому нужно периодическое задание, реализует его контракт; контекст,
которому есть что записать в продуктовый поток, зовёт узкий порт и не узнаёт от
него ничего.
...
Двадцать один: семнадцать доменных и четыре технических — `ml`, `jobs`,
`observability` и `http`. Они ничего не решают про предметную область и
доменными данными не владеют, но границу имеют такую же, поэтому стоят на карте
на общих правах.
...
* `identity_account` — ОДИН ЧЕЛОВЕК НА ВСЮ ПЛОЩАДКУ: отпечаток почты и
  идентификатор, по которому он появляется в нескольких практиках. Единственная
  таблица дерева без `tenant_id`; состав колонок заперт списком
  ([ADR-0019](../adr/0019-one-account-many-practices.md));
* `identity_signup_attempt` — счётчик самостоятельных заведений с одного адреса;
  арендатора нет по существу: заводящийся человек ещё ни в какой практике не
  состоит;
* `identity_tenant` — арендатор: репетитор-одиночка или школа, вместе с тем,
  видна ли практика в подборе; по умолчанию не видна;
...
* `identity_guardian_consent` — согласия на уровни доступа третьего лица:
  строка на уровень, отзыв датой. Единого «родитель видит всё» нет, а записи
  занятий не открываются вместе с опекой
  ([ADR-0006](../adr/0006-parental-access-by-design.md)). Колонка `basis`
  говорит, чем доступ держится: опекой, словом самого ученика или оплатой —
  опека и наблюдение здесь ОДИН механизм, и плательщику схема не даёт ничего,
  кроме счетов ([ADR-0020](../adr/0020-adult-student-is-the-main-case.md));
* `identity_consent` — согласия: на обработку по перечню и на запись занятия.
  Две галочки, а не одна: правовой вес у них разный. Хранится, кто дал, когда,
  какую версию перечня и каким действием; за ребёнка соглашается опекун
...
* `identity_credential` — способ входа: хеш пароля, Argon2id;
* `identity_session` — серверная сессия; отзыв — строка с датой, действующая
  немедленно, а не через срок жизни подписанного токена;
* `identity_one_time_token` — приглашение ученика и сброс пароля: один
  механизм, отпечаток вместо секрета;
* `identity_login_attempt` — счётчик неудачных попыток входа по учётной записи
  и по адресу; в базе, а не в памяти процесса — реплик бывает больше одной.
...
### scheduling — доступность, занятия, серии, отмены, окна, перерывы, подписка
...
* `scheduling_availability_exception` — день, который живёт не по недельному
  правилу: выходной или иные часы;
* `scheduling_calendar_feed` — ПОДПИСКА НА КАЛЕНДАРЬ: отпечаток секрета ссылки
  на ленту ICS и способ называть занятия. Самого секрета в базе нет — утёкшая
  копия не даёт ни одной работающей ссылки. Направление одностороннее: наши
  занятия появляются в чужом календаре, чужие события к нам не переезжают
  ([ADR-0024](../adr/0024-calendar-is-a-subscription-not-an-integration.md),
  [calendar-feed.md](calendar-feed.md));
* `scheduling_time_off` — ПЕРЕРЫВ: отпуск, болезнь, каникулы. Не то же, что
  особый день: тот говорит, когда репетитор работает, и живёт по одному дню, а
  здесь период целиком — с решением по занятиям, необязательной причиной и тем,
  кто его завёл. Отпуск репетитора и каникулы ученика — ОДНА таблица и один
  механизм: перерыв у человека, а какой стороной он стоит в занятии, видно из
  самого занятия ([time-off.md](time-off.md));
...
* `scheduling_lesson_participant` — УЧАСТИЕ: кто занимается, почём, оплачено
  ли и чем кончилось. Отдельной таблицей и с собственными колонками, потому что
  всё это у каждого своё: в группе один платит по своему тарифу, второй по
  своему, а один из троих не пришёл — и занятие всё равно состоялось. Группы
  заложены в модель и не введены
  ([ADR-0023](../adr/0023-group-lessons-modelled-not-implemented.md));
* `scheduling_series` — серия занятий и правило её повторения;
* `scheduling_series_participant` — кто занимается по серии; то же, что у
  занятия, и по той же причине;
* `scheduling_series_exception` — отменённое или перенесённое вхождение серии;
* `scheduling_lesson_history` — кто, что и когда сделал с занятием. Пишется
  только вперёд: спор «я отменял заранее» разрешает строка с моментом, а не
  память участников;
* `scheduling_cancellation` — отмена, её время и причина;
* `scheduling_booking_window` — ПОСЛАБЛЕНИЯ окон для пары репетитор-ученик.
  Сами окна лежат не в базе: умолчания практики приходят из динамического
  конфига и действуют без единой настройки, а в таблице живёт исключение —
  «этому ученику можно свободнее». Сами окна и почему их не настраивают
  обязательно — [booking-windows.md](booking-windows.md).
...
своей транзакции» — это ровно нарушение правила владения, и оно про то, ЧЕЙ
КОД пишет строку, а не про то, чья транзакция открыта.

Разница тонкая и важная, потому что с `PDR-SCHED-06` строка ложится именно в
транзакцию источника — иначе транзакционного outbox не бывает вовсе. Владение
при этом цело: пишет её подписчик оповещений, сидящий на шине обращения, а
расписание не включает ни одного заголовка оповещений и об очереди не знает
(`events::Listeners`, [outbox.md](outbox.md)). Отдельного
`scheduling_outbox` не появилось и не появится: очередь одна, и она у того, кто
её разбирает.
...
### jobs — одиночные периодические задания: блокировка, журнал, следы действий

**Рождается:** фаза 1. Технический контекст.

**Владеет:**

* `jobs_lock` — распределённая блокировка, одна строка на задание во всём
  кластере. Форму таблицы задаёт штатный `storages::postgres::DistLock`, а не
  мы;
* `jobs_run` — последний прогон задания: начало, конец, длительность, исход.
  Возраст этой записи и есть метрика живости задания;
* `jobs_effect` — след уже произведённого действия: `(арендатор, задание, ключ
  действия)`. Из него сделана идемпотентность: повтор после потери блокировки
  упирается в первичный ключ и второго действия не производит.

**Читает:** ничего. Зависимость идёт в обратную сторону: контекст, которому
нужно периодическое задание, реализует контракт `jobs` — что запланировать и
что произвести, — а механизм отвечает за то, чтобы это делал ровно один воркер и
ровно один раз.

**Не образует процесса.** Механизм живёт внутри того сервиса, чьё задание он
крутит: рассылка напоминаний — в `services/main`, пересборка индекса заданий — в
`services/learning`. Отдельного «воркера» в развёртке нет и не будет — иначе
появился бы второй процесс с теми же таблицами и своей выкаткой.

**Доменных данных не хранит.** `jobs_effect` — не факт предметной области, а
отметка «это уже сделано». Арендатор в ней есть, потому что действие всегда
чьё-то, и политика RLS на неё распространяется без исключений: фоновая работа
ходит под арендатором так же, как запрос человека.

**Очередь — другой механизм.** `SKIP LOCKED` решает «много воркеров разбирают
много работ» (`notifications_outbox`, `ml_inference_job`); здесь — «ровно один
воркер во всём кластере делает периодическую работу». Почему это два механизма,
а не один, — [ADR-0011](../adr/0011-single-jobs-on-distlock.md).

### http — форма внешнего контура: отказ, след запроса, идемпотентность

**Рождается:** фаза 1. Технический контекст.

**Владеет:**

* `http_idempotency_key` — ключ идемпотентности клиента: отпечаток тела и
  сохранённый ответ. Повтор с тем же ключом и тем же телом операцию не
  выполняет, а отдаёт сохранённое; с другим телом — отказ, потому что это
  ошибка клиента, а не повтор ([idempotency.md](idempotency.md)).

**Читает:** ничего доменного. У политики он спрашивает решение — публичным
контрактом `identity`, — и содержимого ответа не знает
([http.md](http.md)).

**Не образует процесса.** Форма живёт внутри каждого сервиса, у которого есть
ручки: базовый хендлер, таблица ключей и уборка просроченных едут вместе с ним.
Отдельного «шлюза» в развёртке нет и не будет — иначе появился бы процесс,
через который проходит всё, и падал бы он тоже за всех.

### observability — продуктовые события: поток, обезличивание, срок жизни

**Рождается:** фаза 1. Технический контекст.

**Владеет:**

* `observability_product_event` — поток продуктовых событий: тип, версия схемы,
  арендатор, роль, момент и поля. Идентификатора человека нет ни колонкой, ни
  ключом внутри полей, и это проверяет сама база.

**Читает:** ничего. Порт узкий и односторонний: издатель кладёт событие и ничего
не получает в ответ.

**Не то же, что технические метрики.** Задержки, коды ответов, длины очередей —
это `utils::statistics` userver, и в базу они не попадают вовсе
([ADR-0013](../adr/0013-standard-over-handmade.md)). Здесь другой поток: не
«сколько миллисекунд занял запрос», а «что сделал человек». У них разные
читатели, разные права доступа и разный срок жизни — в одной таблице пришлось бы
выбрать один срок для обоих и ошибиться дважды.

**Обезличен на уровне записи.** Ссылка на арендатора и роль, не на человека.
Отсюда главное правило полей: считает издатель, а не аналитик — «какое это
занятие по счёту у ученика» вычисляет тот, кто знает ученика, и в событие
попадает число. Иначе понадобился бы ключ, связывающий записи одного человека,
то есть тот самый идентификатор
([product-events.md](product-events.md)).

**Событий «на всякий случай» не собирает.** У каждого типа есть вопрос в
[реестре](../product/open-questions.md), без которого он не закрывается;
расхождение роняет сборку (`scripts/check_product_events.py`).

**Процесса не образует,** как и `jobs`: поток пишется изнутри того сервиса, чей
контекст издаёт событие.
...
3. **Таблица без владельца не заводится.** Проверяется машиной:
   ...
   создана мимо карты. Исключение одно — таблицы механизма, а не предметной
   области (`schema_version`, `jobs_lock`, `jobs_run`): у них нет арендатора,
   потому что владеет ими не он, они перечислены поимённо в
   `scripts/migration_model.py` и там же объяснены. Исключение кончается там,
   где появляется `tenant_id`: метатаблица с арендатором проверяется как
   доменная, и это тоже машиной (`scripts/check_rls.py`).
...
* запрос billing обращается к таблице `tariffs`
  (`db/sql/billing/tariff_find_by_code.sql`), а карта требует имени
  `billing_tariff`. Самой таблицы в схеме нет: миграции завели только контекст
  `identity` (`PDR-DB-02`). Имя придёт к владельцу вместе с миграцией,
  заводящей тарифы.

  `scripts/check_table_owners.py` по-прежнему сверяет `create table` в
  миграциях, а не имена таблиц в запросах, — поэтому расхождение записано
  здесь. Но молчаливым оно быть перестало: с `PDR-DB-05` запрос лежит файлом и
  несёт `@no-dto` с причиной («таблицы нет»), а `sql_coverage` называет его
  мёртвым кодом на каждом прогоне ([queries.md](queries.md)).
