...
Двадцать: семнадцать доменных и три технических — `ml`, `jobs` и
`observability`. Они ничего не решают про предметную область и доменными данными
не владеют, но границу имеют такую же, поэтому стоят на карте на общих правах.
