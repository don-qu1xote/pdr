...
flowchart TD
    ...
    identity["identity<br/>фаза 1"]
    scheduling["scheduling<br/>фаза 1"]
    billing["billing<br/>фаза 1 · есть срез"]
    notifications["notifications<br/>фаза 1"]
    ...
    class identity,scheduling,billing,notifications,video,content,notes planned;
    class exercises,practice,media,ml,progress,recommend planned;
    class profile,matching,reputation,chat,business planned;
```

Сплошных узлов на диаграмме нет: **ни один контекст ещё не существует как
модуль**. В дереве есть каркас слоёв (`PDR-ARCH-01`), типы платформы
(`PDR-ARCH-06`) и один демонстрационный срез — тариф и цена пакета
(`libs/pdr-core/src/core/{money,tariff,lesson_package}`,
`application/quote_lesson_package`, адаптер к таблице тарифов). По смыслу срез
принадлежит `billing`; он лежит в общей библиотеке и переезжает в модуль
контекста задачей `PDR-ARCH-07`. Пометка «есть срез» на узле `billing` — про
это и только про это.
...
3. **Таблица без владельца не заводится.** Миграция, добавляющая таблицу, чьё
   имя не начинается с имени контекста-владельца, — ошибка ревью.
4. **Триггер выделения без наблюдаемого условия не считается триггером.**
...
* демонстрационный срез из `PDR-ARCH-01` обращается к таблице `tariffs`
  (`libs/pdr-core/src/infrastructure/postgres_tariff_repository.cpp`), а карта
  называет её владельца `billing` и требует имени `billing_tariff`. Самой
  таблицы в дереве нет — миграций ещё нет вовсе. Имя приводится к владельцу
  задачей области `DB`, вместе с первой миграцией.
