...
enum class Tie : std::uint8_t {
    ...
    /// Ресурс о нём самом.
    kAboutMe,
    /// Ресурс о том, за кем он смотрит: о подопечном ребёнке или о взрослом,
    /// который сам назвал его наблюдателем. ОДНО ОТНОШЕНИЕ НА ОБА СЛУЧАЯ —
    /// потому что и механизм один, разные у них только основания
    /// (`identity::ConsentBasis`).
    kInMyCare,
...
///
/// «Смотрит за ним» приходит готовым ответом, а не запросом отсюда: считать это
/// умеет только тот, у кого есть хранилище (`policies::SubjectBuilder`).
