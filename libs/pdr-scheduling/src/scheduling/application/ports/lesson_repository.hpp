...
public:
    ...
    /// Состояние занятия после разрешённого перехода. Переход разрешил домен;
    /// здесь только запись, и машина состояний остаётся в одном месте.
    virtual core::Result<void> SetState(const Lesson& lesson) = 0;

    /// Занятие на новом месте. ТА ЖЕ СТРОКА, тот же идентификатор — перенос не
    /// заводит второго занятия, иначе связь с оплатой и прогрессом рвётся.
    ///
    /// Отказать может так же, как и сохранение: на новом месте у репетитора
    /// может уже стоять другое занятие, и ловит это то же ограничение базы.
    virtual core::Result<void> Move(const Lesson& lesson) = 0;
