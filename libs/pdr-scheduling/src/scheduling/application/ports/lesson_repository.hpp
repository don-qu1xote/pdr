...
/// Занятия: два запроса и сохранение. Больше ничего.
///
/// ЗАПРОСОВ РОВНО ДВА, И ОБА НАЗВАНЫ. Метода `FindByCriteria(const Criteria&)`
/// здесь нет и не будет: он возвращает абстракцию, которая протекает, — под
/// «критерием» рано или поздно оказывается SQL, собранный на слое сценариев, и
/// индексы под него завести нечем, потому что запрос заранее неизвестен.
/// Названный запрос знает свой индекс: `scheduling_lesson_by_tutor` и
/// `scheduling_lesson_by_participant` заведены под эти два и ни подо что ещё.
