...
struct LessonCancelled final {
    ...
    /// КОГО ЭТО КАСАЕТСЯ. Оба — и тот, кто отменил, тоже: подписчику нужно
    /// написать обоим, а спросить у расписания, кто занимался, ему нечем.
    /// Событие, из которого получателя не видно, заставляет подписчика ходить
    /// за ним в чужой контекст — и связь, которой не должно быть, появляется.
    core::PersonId tutor;
